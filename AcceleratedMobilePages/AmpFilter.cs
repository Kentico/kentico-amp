...
namespace AcceleratedMobilePages
{
    class AmpFilter
    {
        ...
        {
            int state = CheckStateHelper.CheckFilterState();
        ...
        private string TransformToAmpHtml(string finalHtml)
        {
            ...
            RemoveRestrictedElements(ref doc);
            ResolveComplexElements(ref doc);
            ReplaceRegularTagsByAmpTags(ref doc);
        ...
        /// </summary>
        /// <param name="doc">Reference to complete HtmlDocument</param>
        private void RemoveRestrictedElements(ref HtmlDocument doc)
        {
            // Remove restricted elements
            foreach (var rule in Constants.XPATH_RESTRICTED_ELEMENTS)
            {
                RemoveElement(ref doc, rule);
            }

            // Remove attributes
            RemoveAttribute(ref doc, Constants.XPATH_ATTR_STYLE, Constants.XPATH_ATTR_STYLE_NAME);
        }
        ...
        /// </summary>
        /// <param name="doc">Reference to complete HtmlDocument</param>
        private void ResolveComplexElements(ref HtmlDocument doc)
        {
            ...
            if (nodes != null)
            {
                ...
                customElements += ampCustomForm + Constants.NEW_LINE;
            }
        ...
        /// </summary>
        /// <param name="doc">Reference to complete HtmlDocument</param>
        private void ReplaceRegularTagsByAmpTags(ref HtmlDocument doc)
        {
            // Script tags source URLs from settings
            ...
            ReplaceElement(ref doc, Constants.XPATH_IMG, Constants.XPATH_IMG_REPLACEMENT);
            if (ReplaceElement(ref doc, Constants.XPATH_VIDEO, Constants.XPATH_VIDEO_REPLACEMENT))
            {
                customElements += ampCustomVideo + Constants.NEW_LINE;
            }
            if (ReplaceElement(ref doc, Constants.XPATH_AUDIO, Constants.XPATH_AUDIO_REPLACEMENT))
            {
                customElements += ampCustomAudio + Constants.NEW_LINE;
            }
            if (ReplaceElement(ref doc, Constants.XPATH_IFRAME, Constants.XPATH_IFRAME_REPLACEMENT))
            {
                customElements += ampCustomIframe + Constants.NEW_LINE;
            }
        }
        ...
        private string InsertCompulsoryMarkupAndCss(string finalHtml)
        {
            ...
            // This link points to the regular HTML version of AMP page
            ...
            // Concatenate all the compulsory markup together with CSS styles
            string compulsoryMarkup = headTag + Constants.NEW_LINE +
            ...
            // Insert compulsory markup
            finalHtml = Regex.Replace(finalHtml, Constants.REGEX_HEAD, compulsoryMarkup);
            return finalHtml;
        }
        ...
        private string GetStylesheetText()
        {
            ...
            if (q.FirstObject.UseDefaultStylesheet)
            {
                ...
                // Using default AMP CSS stylesheet
                {
            ...
            // Using specific AMP CSS set for this page
            {
        ...
        private void RemoveElement(ref HtmlDocument doc, string elementXPath)
        {
        ...
        private bool ReplaceElement(ref HtmlDocument doc, string xPath, string replacement)
        {
        ...
        private void RemoveAttribute(ref HtmlDocument doc, string xPath, string attrName)
        {
            var nodes = doc.DocumentNode.SelectNodes(xPath);
            if (nodes != null)
            {
